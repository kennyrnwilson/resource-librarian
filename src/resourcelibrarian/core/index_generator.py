...
class IndexGenerator:
    ...
    def generate_authors_index(self, catalog: LibraryCatalog) -> None:
        ...
        for author in sorted(books_by_author.keys()):
            ...
            for book in books:
                ...
                rel_path = f"../{book.folder_path.relative_to(self.library_root / 'books')}/index.md"
    ...
    def generate_channels_index(self, catalog: LibraryCatalog) -> None:
        ...
        for channel in sorted(videos_by_channel.keys()):
            ...
            for video in videos:
                ...
                # Get emoji tags
                emoji_tags = " ".join([f"{cat}" for cat in video.manifest.categories if len(cat) <= 2])
                if not emoji_tags:
                    emoji_tags = ""
    ...
    def generate_video_titles_index(self, catalog: LibraryCatalog) -> None:
        ...
        for letter in sorted(videos_by_letter.keys()):
            ...
            for video in videos:
                # Create relative path
                rel_path = f"../{video.folder_path.relative_to(self.library_root / 'videos')}/index.md"
    ...
    def generate_channel_indices(self, catalog: LibraryCatalog) -> None:
        ...
        for channel_dir, videos in videos_by_channel_dir.items():
            ...
            for video in sorted_videos:
                ...
                # Add video link
                lines.append(f"[Watch on YouTube](https://www.youtube.com/watch?v={video.video_id})")
                lines.append("")
                lines.extend(["---", ""])
    ...
    def generate_book_index(self, book: Book) -> None:
        ...
        lines = [
            ...
            "---",
            "",
            f"**Browse:** [📚 Library Home](../../../README.md) • [👤 By Author](../../_index/authors.md) • [🔤 By Title](../../_index/titles.md) • [🏷️ By Category](../../../categories/index.md)",
            "",
            "---",
            "",
        ]
        ...
        if book.categories:
            cat_links = [f"[{cat}](../../../categories/index.md#{cat.lower().replace(' ', '-')})" for cat in book.categories]
        ...
        if chapters_dir.exists():
            ...
            if chapter_files:
                ...
                lines.append("")
                lines.append(f"Chapters are available in the [`full-book-formats/chapters/`](full-book-formats/chapters/) directory.")
                lines.append("")
    ...
    def generate_video_index(self, video: Video) -> None:
        ...
        lines = [
            ...
            "---",
            "",
            f"**Browse:** [📚 Library Home](../../../README.md) • [📺 By Channel](../../_index/channels.md) • [🔤 By Title](../../_index/titles.md) • [🏷️ By Category](../../../categories/index.md)",
            "",
            "---",
            "",
        ]
        ...
        if video.manifest.categories:
            cat_links = [f"[{cat}](../../../categories/index.md#{cat.lower().replace(' ', '-')})" for cat in video.manifest.categories]
            lines.append(f"**Categories:** {', '.join(cat_links)}")
            lines.append("")
